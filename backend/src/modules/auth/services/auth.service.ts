...
import type { AuthData, LoginRequest, RegisterRequest, SafeUser, SessionMetadata } from '../types/auth.types.js';
...
const createAuthSession = async (user: UserDocument, metadata: SessionMetadata): Promise<AuthData & { refreshToken: string }> => {
...
): Promise<AuthData & { refreshToken: string }> => {
...
export const refreshUserSession = async (refreshToken: string): Promise<AuthData & { refreshToken: string }> => {
